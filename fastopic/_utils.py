...
class ScDataset:
    """Dataset class for single-cell data that works with precomputed cell embeddings."""
    ...
    def __init__(
        ...
        # Use real gene expression data as BOW matrix
        import scipy.sparse as sp
        if expression_bow is not None:
            # Use provided real gene expression BOW matrix
            self.train_bow = expression_bow
        else:
            # Fallback: create a simple identity-like BOW matrix for genes
            # This is a placeholder - in practice you should always provide real expression data
            n_cells = cell_embeddings.shape[0]
            n_genes = len(gene_names)
            self.train_bow = sp.csr_matrix(np.ones((n_cells, n_genes)) / n_genes)
        
        self.vocab = gene_names
        self.vocab_size = len(self.vocab)
        
        self.cell_embeddings = cell_embeddings.astype(np.float32)
        self.cell_embed_size = self.cell_embeddings.shape[1]
